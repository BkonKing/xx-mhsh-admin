    ...
    <div>
      <a-card style="margin-top: 24px" :bordered="false" title="基础信息">
        <a-descriptions v-if="infoData.data">
          ...
          <a-descriptions-item label="业主" span="2"><a v-if="infoData.data.uid > 0" :href="`/xmht/household/member/getMemberList?uid=${infoData.data.uid}`" target="_blank">{{ infoData.data.realname + ' ' + infoData.data.mobile }}</a><template v-else>{{ infoData.data.realname + ' ' + infoData.data.mobile }}</template></a-descriptions-item>
